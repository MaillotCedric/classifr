$(document).ready(function() {
    ajax_call("GET", "../api/modele", donnees={}, success_callback=afficher_dernier_modele, error_callback=afficher_error);
});

function afficher_dernier_modele(data) {
    var dernier_modele = data.results.reverse()[0];;
    console.log(dernier_modele);

    var modeles_container = document.getElementById("modeles-container");

    // Convertir la date au format "dd-mm-yyyy"
    let date_created = new Date(dernier_modele.date_created);
    ...
    console.log(date_created_formatted);

    modeles_container.innerHTML = `
        <div class="modele-thumbnail">
        <h1>${dernier_modele.nom}</h1>
    ...
    // Afficher les jauges
    var precision_value = dernier_modele.accuracy;
    var recall_value = dernier_modele.recall;
    ...
    let id_modele = dernier_modele.id;
